...
class IgViewModel @Inject constructor(
    ...
    private fun getUserData(uid: String) {
        ...
        db.collection(USERS).document(uid).get()
            .addOnSuccessListener {
                ...
                refreshPosts()
                getPersonalizeFeed()
                user?.userId?.let { it1 -> getFolloweres(it1) }
            }
            .addOnFailureListener { exc ->
    ...
    private fun getGeneralFeed() {
        ...
        val different = 24 * 60 * 60 * 1000 // 1 day in millis
        db.collection(POSTS)
            .whereGreaterThan("time", currentTime - different)
            .get()
            .addOnSuccessListener {
                convertPosts(documents = it, outState = postsFeed)
                postsFeedProgress.value = false
            ...
            postData.likes?.let { likes ->
                ...
                postData.postId?.let { postId ->
                    db.collection(POSTS).document(postId).update("likes", newLikes)
                        ...
                        .addOnFailureListener {
                            handleException(it, "Unable to likes post")
                        }
                }
            }
        }
    ...
    private fun getFolloweres(uid: String) {
